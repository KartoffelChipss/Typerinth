...
export default class GetProjectVersionsRoute extends Route<ProjectVersion[]> {
    ...
    getUrl(): URL {
        const url = Route.addPathSegment(
            this.baseUrl,
            `/project/${this.projectId}/version`
        );
        if (this.options.loaders)
            url.searchParams.append(
                'loaders',
                `[${this.options.loaders?.map((l) => '"' + l + '"').join(',') ?? ''}]`
            );
        if (this.options.game_versions)
            url.searchParams.append(
                'game_versions',
                `[${this.options.game_versions?.map((gv) => '"' + gv + '"').join(',') ?? ''}]`
            );
        if (!!this.options.featured)
            url.searchParams.append(
                'featured',
                this.options.featured?.toString()
            );
        return url;
    }
